...
public enum VcsProperties implements CiOption {
    ...
    public static boolean isSnapshotRef(final String gitRef) {
        return gitRef != null && !gitRef.isEmpty()
            && (Constants.GIT_REF_NAME_DEVELOP.equals(gitRef)
            || gitRef.startsWith(Constants.GIT_REF_PREFIX_FEATURE));
    }

    public static boolean isReleaseRef(final String gitRef) {
        return gitRef != null && !gitRef.isEmpty()
            && (gitRef.startsWith(Constants.GIT_REF_PREFIX_HOTFIX)
            || gitRef.startsWith(Constants.GIT_REF_PREFIX_RELEASE)
            || gitRef.startsWith(Constants.GIT_REF_PREFIX_SUPPORT));
    }
